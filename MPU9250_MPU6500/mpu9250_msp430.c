...
void Mpu_Convert_Data(struct Sensor *sensor){
	// gyro convert
    sensor->gyro_x_float = ((int)sensor->gyro_x[0] << 8) | sensor->gyro_x[1];    // [0] is high bit, [1] is low bit
//  sensor->gyro_x_float = (float)sensor->cov_tmp * sensor->gyro_reso;
    sensor->gyro_y_float = ((int)sensor->gyro_y[0] << 8) | sensor->gyro_y[1];    // [0] is high bit, [1] is low bit
//  sensor->gyro_y_float = (float)sensor->cov_tmp * sensor->gyro_reso;
    sensor->gyro_z_float = ((int)sensor->gyro_z[0] << 8) | sensor->gyro_z[1];    // [0] is high bit, [1] is low bit
//  sensor->gyro_z_float = (float)sensor->cov_tmp * sensor->gyro_reso;
    // accel convert
    sensor->accel_x_float = ((int)sensor->accel_x[0] << 8) | sensor->accel_x[1];    // [0] is high bit, [1] is low bit
//  sensor->accel_x_float = (float)sensor->cov_tmp / sensor->accel_reso;
    sensor->accel_y_float = ((int)sensor->accel_y[0] << 8) | sensor->accel_y[1];    // [0] is high bit, [1] is low bit
//  sensor->accel_y_float = (float)sensor->cov_tmp / sensor->accel_reso;
    sensor->accel_z_float = ((int)sensor->accel_z[0] << 8) | sensor->accel_z[1];    // [0] is high bit, [1] is low bit
//  sensor->accel_z_float = (float)sensor->cov_tmp / sensor->accel_reso;

    sensor->temp_float = ((int)sensor->temp[0] << 8) | sensor->temp[1];    // [0] is high bit, [1] is low bit
    //mag convert
...
void Mpu_Init(){
	I2C_Master_Init(TRUE, SMCLKFREQ, I2C_CLOCK_FREQ);
	Mpu_9250_Init();
...
void Mpu_9250_Init(){
	...
	int i=0;
	for(i=0;i<100;i++)
		__delay_cycles(1000);					// delay 0.1s
...
void Mpu_I2c_AK8963C_Init(){
	Mpu_I2c_SetReg_Mag(AK8963_CNTL, 0x01);  // Power down magnetometer
	//int i=0;
	//for(i=0;i<100;i++)
//	    __delay_cycles(8000);
  //  Mpu_I2c_SetReg_Mag(AK8963_CNTL, 0x0c);  // Power down magnetometer
...
int Mpu_I2c_Read_Temp(unsigned char *buffer){
       Mpu_I2c_ReadReg(TEMP_OUT_H, 1, buffer);
       Mpu_I2c_ReadReg(TEMP_OUT_L, 1, buffer+1);     // read Accel,
       return 1;
}

void Mpu_I2c_ReadAccel(unsigned char *x, unsigned char *y, unsigned char *z, unsigned char *Temp, unsigned char *st){
	Mpu_I2c_ReadAccel_x(x);
	Mpu_I2c_ReadAccel_y(y);
	Mpu_I2c_ReadAccel_z(z);
	Mpu_I2c_Read_Temp(Temp);
	Mpu_I2c_ReadAndClearIntStatusRegister(st);
}
