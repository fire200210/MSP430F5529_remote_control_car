				 	 ...
				 	 unsigned long desiredI2CClock)
{
	...
	P3SEL |= BIT0;             // use UCB0SDA, P2.1
	P3SEL |= BIT1;				// use UCB0SCL, P2.2
//	P2DIR &= ~BIT7;
//	P2REN = BIT7;
...
/**
* @brief <b>Function Name</b>:     : I2C_Write_Packet_To_Gyro
* @brief  <b>Description</b>: Sends data packet to gyroscope
* @param Input Parameters:
* <BR> unsigned char <b>altiRegName</b> is the register the MSP430 will write
* data to
* <BR> unsigned char <b>dataLength</b> is the number of data bytes that will be
* written
* <BR> unsigned char * <b>writeData</b> is the address where the data bytes to
* be written are
* @return Return Values: None
**/
void I2C_Write_Packet_To_Gyro(unsigned char gyroRegName,
...
void I2C_Write_Packet_To_Mag(unsigned char gyroRegName,
						    unsigned char dataLength,
						    unsigned char *writeData)		// actually only diff is address, 2 functions because no params for i2c address
{
  // Assign values to global variables
  tx_reg_name = gyroRegName;
  tx_length = dataLength;
...
/**
* @brief <b>Function Name</b>:     : I2C_Read_Packet_From_Gyro
* @brief  <b>Description</b>: Reads data packet from gyroscope
* @param Input Parameters:
* <BR> unsigned char <b>altiRegName</b> is the register the MSP430 will read data
* from
* <BR> unsigned char <b>dataLength</b> is the number of data bytes that will be
* read
* <BR> unsigned char * <b>readData</b> is the address where the data bytes to be
* read will be
* @return Return Values: None
**/
void I2C_Read_Packet_From_Gyro(unsigned char gyroRegName,
					         unsigned char dataLength,
...
void I2C_Read_Packet_From_Mag(unsigned char gyroRegName,
					         unsigned char dataLength,
					         unsigned char *readData)
{
  // Assign values to global variables
  ...
  tx_byte_ctr = 1;
  rx_byte_ctr = 0;
  //Mpu_I2c_SetReg_Mag(AK8963_CNTL, 0x0C);  // Enter Fuse ROM access mode
